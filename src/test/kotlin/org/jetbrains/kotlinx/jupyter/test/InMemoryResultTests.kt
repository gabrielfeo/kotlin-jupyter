...
class InMemoryResultTests : AbstractSingleReplTest() {
    ...
    private fun doInMemoryTest(
        ...
        result.renderedValue.shouldBeInstanceOf<MimeTypedResult>()
        result.displayValue.shouldBeInstanceOf<MimeTypedResult>()
        val display = result.displayValue as MimeTypedResult
        display.size.shouldBe(2)
        display.shouldContainKey("image/png")
        display.shouldContainKey(InMemoryMimeTypes.SWING)
